    ...
    this.addEventListener("load", function () {
      if (this._url.indexOf && this._url.indexOf("options_menu/?button_id=") > -1) {
        ...
        try {
          ...
          requestParams = requestParams.slice(
            requestParams.indexOf("id="),
      ...
      //            /* Request body  */ postData
        ...
        if (waistIndex > -1) {
          ...
          try {
            ...
            const advertiserName = this.responseText
              .slice(adv_index, this.responseText.length - 1)
              .match(re_advertiserName)[1];
            ...
            console.log("advertiser name parsing error", e);
          }
        }
        ...
        if (waistButtonIndex > -1) {
          ...
          let chunk = this.responseText.slice(
            this.responseText.indexOf("StoryEmbedPostMenuItem"),
            this.responseText.indexOf("StoryEmbedPostMenuItem") + 800
          );
          chunk = chunk
            .replace(/\\u00252F/g, "")
            .replace(/\\u00253A/g, "")
          ...
          if (!chunk || !vanity || !postId) {
            chunk = this.responseText.slice(
              this.responseText.indexOf("StoryWhyAmISeeingThisAdMenuItem"),
              this.responseText.indexOf("StoryWhyAmISeeingThisAdMenuItem") + 1000
            );
            chunk = chunk
              .replace(/\\u002522/g, "")
              .replace(/\\u00252C/g, "")
        ...
        if (sponsoredIndex > -1) {
          ...
          let txt = this.responseText.slice(
            this.responseText.indexOf(preStart) + 20,
            this.responseText.indexOf("jsmods") - 2
          );
          ...
          for (let i = 0; i < 3; i++) {
            if (txt.length > 50) {
              ...
              let clientToken = chunk.match(re_EgoClientToken)
                ? chunk.match(re_EgoClientToken)[1]
                : null;
              const asyncParamsAd = JSON.parse(JSON.stringify(asyncParams));
              asyncParamsAd["variables"] = `{"adId": "${adId}", "clientToken": "${clientToken}"}`;
              const data = {
