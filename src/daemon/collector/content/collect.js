...
function addToFrontAdQueue(ad) {
  ...
  const nextNum =
    Math.max.apply(
      null,
      Object.keys(frontadqueue).map(function (x) {
        return parseInt(x);
      })
    ) + 1;
    ...
    setTimeout(function () {
      ...
      if (menus) {
        for (let j = 0; j < menus.length; j++) {
          const isMenuExpanded = hasUserExpandedMenu(buttonsWithExpandableMenus[j]);
...
function checkLinkAndText(elt) {
  ...
  if (!elt) {
    ...
    (elt && elt.hasAttribute("href") && elt.getAttribute("href") === "#") ||
...
function filterFrontAds(lst, politLst) {
  ...
  for (const elt of lst) {
    ...
    if (
      ajaxify &&
      ajaxify.indexOf(politAdSubtitle) > -1 &&
    ...
    if (
      sponsoredText.indexOf(elt.text) > -1 &&
    ...
    if (
      sponsoredText.indexOf(elt.getAttribute("aria-label")) > -1 &&
      isScrolledIntoView(elt, layoutStyle)
    ) {
      newLst.push(elt);
    }
...
function filterSheets(sheets) {
  ...
  for (let i = 0; i < sheets.length; i++) {
    if (
      sheets[i].href &&
      (sheets[i].href.indexOf("https://www.facebook.com/rsrc.php") > -1 ||
...
function getNonHiddenTextByChildren(children) {
  ...
  for (let i = 0; i < children.length; i++) {
    if (
      getComputedStyle(children[i])["font-size"] === "0px" ||
      getComputedStyle(children[i])["opacity"] === "0"
    ) {
      continue;
    }
...
function findFrontAdsWithHiddenLetters(layoutStyle) {
  ...
  for (let i = 0; i < elems.length; i++) {
    if (
      isLinkSponsoredHiddenLetters(elems[i]) &&
      !isSponsoredLinkHidden(elems[i]) &&
      isScrolledIntoView(elems[i], layoutStyle)
    ) {
      links.push(elems[i]);
    }
  }
...
function getFrontAdFrames() {
  ...
  const frontAds = links.map((link) =>
    layoutStyle === "FB5" ? getParentAdDivNewStyle(link) : getParentAdDiv(link)
  );
    ...
    .then((result) => {
      if (result) {
        ...
        if (!layoutStyle) {
          ...
          api
            .post("log/raw", { json: finalPayload })
            .then((response) => {
...
function sendRationale(postData) {
  ...
  const sideAd =
    advertiserName && adData && adData.fbStoryId && adData.fbStoryId.indexOf(adId) > -1;
  if (sideAd) {
    // FB4 or FB5 side ad
  ...
  if (adNew) {
    ...
    api.post("log/raw", { json: finalPayload }).then((response) => {
      // response completed, no log
    });
  ...
  api.post("log/raw", { json: finalPayloadRationale }).then((response) => {
    // response completed, no log
  });
}
