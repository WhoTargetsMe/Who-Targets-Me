...
const fetchRationale = (advertId) => {
  console.log('fetchRationale advertId-1', advertId)
...
const rationales = (advertIdQueue, group) => {
  console.log("FBADVERTRATIONALE-1", advertIdQueue, group)
    ...
    return fetchRationale(advert.fbAdvertId)
      .then(parsedRationale => {
        ...
        if (parsedRationale) {
          ...
          console.log("FBADVERTRATIONALE-3 (if parsed)", advert.fbAdvertId,
              ...
              try {
                ...
                if (domSponsored.length === 0) { // if there's a timestamp, it's not an ad
                  ...
                  if (fbStoryId === advert.fbStoryId) {
                    ...
                    $(`<div style="color:black; padding: 15px; font-weight:bold;min-height: 30px; background-color:orange; text-align:left; padding-top:10px;border:2px solid grey;">${rationaleText}</div>`).appendTo(nextNode);
      ...
      .then((result) => {
        ...
        return Promise.all([newAds(fbStoryIds, group), rationales(rationale.advertIdQueue, group)])
          .then(results => {
            ...
            // advertIds = advertIds.filter(id => id.fbAdvertId !== null);
            // const validIds = advertIds.map(id => id.fbStoryId);
            // adverts = adverts.filter(adv => validIds.includes(adv.related));
            // console.log('VERY IMPORTANT!2 adverts', adverts)
            // console.log('VERY IMPORTANT! validIds', validIds)
